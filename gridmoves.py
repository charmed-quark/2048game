...
def move_left(grid):
    ...
    for i in range(len(matrix)):
        matrix[i] = compress_list_and_pad(matrix[i], "left")
        matrix[i] = merge_left(matrix[i])
    return matrix.tolist()

def move_right(grid):
    matrix = np.asarray(grid)
    for i in range(len(matrix)):
        matrix[i] = compress_list_and_pad(matrix[i], "right")
        matrix[i] = merge_right(matrix[i])
    return matrix.tolist()

def move_up(grid):
    matrix = np.transpose(move_left(np.transpose(grid)))
    return matrix.tolist()        # this syntax is bc numpy works with optimized arrays and not regular lists

def move_down(grid):
    matrix = np.transpose(move_right(np.transpose(grid)))
    return matrix.tolist()
...
extra space padded.
"""
def compress_list_and_pad(lst, shift_elems_to):
    original_length = len(lst)
    lst = compress_list(lst)
    try:
        if len(lst) < original_length:
            if shift_elems_to.lower() == "left":
                lst[len(lst):] = [0]*(original_length - len(lst))
...
def merge_left(lst):
    ...
    return compress_list_and_pad(lst, "left")
...
def merge_right(lst):
    ...
    for i in range(len(lst)-1, 0, -1):
        if lst[i] == lst[i-1] and lst[i] != 0:
            ...
            lst[i-1] = 0
    # get rid of remaining nones
    return compress_list_and_pad(lst, "right")
